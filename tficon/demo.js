...
function changeImage() {
    let hat = hats[cur_hat].textContent.toLowerCase().trim();
    if (hat === '...') {
        hat = "default";
    }
    let animal = animals[cur_animal].textContent.toLowerCase().trim();
    // Extracting the animal name from the <img> alt attribute
    if (animals[cur_animal].getElementsByTagName("img")[0]) {
        animal = animals[cur_animal].getElementsByTagName("img")[0].alt.toLowerCase().trim();
    }
    image.src = "tficon/" + animal + "_" + hat + ".png";
}
